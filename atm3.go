...
func withdraw(n ...int) { //defining a function to withdraw money
	...
	if WdwAmount%100 != 0 || WdwAmount <= 0 { //checks if the amount entered by the user is not a multiple of 100 and not less than or equal to zero
		...
		if WdwAmount <= amount { //if the withdrawal amount is less than or equal to amount execute
			...
			for j := 0; j < len(n); j++ { //creating a for loop with  condition: length less than n
				if WdwAmount >= n[j] { //if the withdrawing amount is greater than or equal to  item in the slice for j(eg:1800>=2000)
			...
			for j := 0; j < len(n); j++ { //define a for loop with  condition: length less than n
...
func main() {
	withdraw(2000, 500, 200, 100) //calling the function withdraw by passing arguments
	//Max 5 transactions per day
	for i := 0; i < 4; i++ { //defining a for loop
		fmt.Println("\nDo you want to continue?Press 1 to continue or Press 2 to cancel\n") //printing the message asking to continue withdrawal or not
		fmt.Scanln(&choice)                                                                 //scan the value of type int

		if choice == 1 { // if the value is 1
			withdraw(2000, 500, 200, 100) //if the condition is true call the function
